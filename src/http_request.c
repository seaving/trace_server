...
static httpRequest_result_t _httpRequest_recv_header(
		client_t *client, httpRequest_t *httpRequest)
{
	int len = 0;
	char ch;
	...
	for ( ; ; )
	{
		len = evserver_recv_client(client, &ch, 1);
		if (len < 0)
		{
		...
		client_t *client, httpRequest_t *httpRequest)
{
	...
	for ( ; ; )
	{
		...
		read_len = evserver_recv_client(client, 
...
httpRequest_result_t httpRequest_recv_header(
		client_t *client, httpRequest_t *httpRequest)
{
	return _httpRequest_recv_header(client, httpRequest);
}
		...
		client_t *client, httpRequest_t *httpRequest)
{
	...
	{
		return _httpRequest_recv_context(client, httpRequest);
	}
